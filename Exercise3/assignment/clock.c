...
int lru(int* ref_arr, size_t ref_arr_sz, size_t frame_sz) {
    ...
    // TODO
    // Init circular queue
    char* clock_queue = (char*) malloc(sizeof(char) * frame_sz);
    for (i = 0; i < frame_sz; ++i) clock_queue[i] = 0;
    int clock_queue_head = 0;
    ...
    for (i = 0; i < ref_arr_sz; ++i)
    {
        ...
        if (is_fault)
        {
            while (clock_queue[clock_queue_head] != 0)
            {
                clock_queue[clock_queue_head] = 0;
                clock_queue_head = (clock_queue_head + 1) % frame_sz;
            }
            frames[clock_queue_head] = ref_arr[i];
            clock_queue[clock_queue_head] = 1;
